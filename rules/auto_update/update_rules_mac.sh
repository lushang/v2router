...
# delete site below
sed -i -e '/m-team/d' "gfwlist1.conf"
sed -i -e '/windowsupdate/d' "gfwlist1.conf"
sed -i -e '/v2ex/d' "gfwlist1.conf"
sed -i -e '/apple\.com/d' "gfwlist1.conf"

md5sum1=$(md5 gfwlist1.conf | sed 's/ /\n/g' | sed -n 4p)
md5sum2=$(md5 ../gfwlist.conf | sed 's/ /\n/g' | sed -n 4p)
...
md5sum3=$(md5 chnroute1.txt | sed 's/ /\n/g' | sed -n 4p)
md5sum4=$(md5 ../chnroute.txt | sed 's/ /\n/g' | sed -n 4p)
...
if [ "$md5sum3"x = "$md5sum4"x ]; then
	...
	sed -i "2c $(date +%Y-%m-%d) # $md5sum3 chnroute" -e ../version1
fi
echo =================
# ======================================
...
md5sum5=$(md5 cdn1.txt | sed 's/ /\n/g' | sed -n 4p)
md5sum6=$(md5 ../cdn.txt | sed 's/ /\n/g' | sed -n 4p)
...
if [ "$md5sum5"x = "$md5sum6"x ]; then
	...
	sed -i "4c $(date +%Y-%m-%d) # $md5sum5 cdn" -e ../version1
fi
echo =================
# ======================================
# use apnic data
curl -4 -o- http://ftp.apnic.net/apnic/stats/apnic/delegated-apnic-latest >apnic.txt
cat apnic.txt | awk -F\| '/CN\|ipv4/ { printf("%s/%d\n", $4, 32-log($5)/log(2)) }' >chnroute1.txt
...
md5sum9=$(md5 Routing.txt | sed 's/ /\n/g' | sed -n 4p)
md5sum10=$(md5 ../Routing.txt | sed 's/ /\n/g' | sed -n 4p)
echo =================
if [ "$md5sum9"x = "$md5sum10"x ]; then
	...
	echo update Routing!
	cp Routing.txt ..
	sed -i "5c $(date +%Y-%m-%d) # $md5sum9 Routing" -e ../version1
fi
echo =================
# ======================================
sed 's|/114.114.114.114$||' accelerated-domains.china.conf >WhiteList_tmp.txt
sed -i -e 's|\(\.\)|\\\1|g' WhiteList_tmp.txt
sed -i -e 's|server=/|.*\\\b|' WhiteList_tmp.txt
sed -i -e 's|b\(cn\)$|\.\1|' WhiteList_tmp.txt
...
md5sum7=$(md5 WhiteList.txt | sed 's/ /\n/g' | sed -n 4p)
md5sum8=$(md5 ../WhiteList.txt | sed 's/ /\n/g' | sed -n 4p)
echo =================
if [ "$md5sum7"x = "$md5sum8"x ]; then
	...
	sed -i "6c $(date +%Y-%m-%d) # $md5sum7 WhiteList" -e ../version1
fi
echo =================
...
md5sum11=$(md5 WhiteList_new.txt | sed 's/ /\n/g' | sed -n 4p)
md5sum12=$(md5 ../WhiteList_new.txt | sed 's/ /\n/g' | sed -n 4p)
echo =================
if [ "$md5sum11"x = "$md5sum12"x ]; then
	...
	sed -i "7c $(date +%Y-%m-%d) # $md5sum11 WhiteList_new" -e ../version1
fi
echo =================
...
md5sum13=$(md5 apple_download.txt | sed 's/ /\n/g' | sed -n 4p)
md5sum14=$(md5 ../apple_china.txt | sed 's/ /\n/g' | sed -n 4p)

md5sum15=$(md5 google_download.txt | sed 's/ /\n/g' | sed -n 4p)
md5sum16=$(md5 ../google_china.txt | sed 's/ /\n/g' | sed -n 4p)
...
if [ "$md5sum13"x = "$md5sum14"x ]; then
	...
	sed -i "8c $(date +%Y-%m-%d) # $md5sum13 apple_china" -e ../version1
fi
if [ "$md5sum15"x = "$md5sum16"x ]; then
	...
	sed -i "9c $(date +%Y-%m-%d) # $md5sum15 google_china" -e ../version1
fi
echo =================
# ======================================
